<template>
  ...
  >
    ...
    <div class="search" style="line-height: 30px">
      <a style="padding: 10px">@用户管理</a>
      <a style="float: right" th:href="@{/user/toAdd}" class="btn btn-success"
        >添加</a
      >
      <div
      ...
      <table class="table table-bordered">
        ...
        <tbody>
          <tr th:each="item : ${items}">
            ...
            <!--    <td th:text="${T(com.sys.bizclient.DictionaryUtils).getValue('sex',item.sex)}"></td> -->
            <td th:text="${item.phone}"></td>
            <td th:text="${item.address}"></td>
            <!--   <td th:text="${T(com.sys.bizclient.DictionaryUtils).getValue('role',item.role)}"></td>
            <td th:text="${T(com.sys.bizclient.DictionaryUtils).getValue('status',item.status)}"></td> -->
            <td>
              <a
                class="btn btn-sm btn-warning"
                th:href="@{/user/toEdit(id=${item.id})}"
                >修改</a
              >
              <a
                class="btn btn-sm btn-danger"
                th:href="@{/user/delete(id=${item.id})}"
                >删除</a
              >
            </td>
          </tr>
    ...
    <div class="modal-footer">
      <ul class="pagination pagination-sm">
        <!-- 首页 -->
        <li>
          <a th:href="'/user/list?pageNum=0'">首页</a>
        </li>
        <!-- 上一页 -->
        <li th:if="${items.hasPrevious()}">
          <a
            th:href="'/user/list?pageNum=' + ${items.previousPageable().getPageNumber()}"
            th:text="上一页"
          ></a>
        </li>
        <!-- 中间页 -->
        ...
        <li th:if="${items.hasNext()}">
          <a
            th:href="'/user/list?pageNum=' + ${items.nextPageable().getPageNumber()}"
            th:text="下一页"
          ></a>
        </li>
        <!-- 尾页 -->
        <li>
          <a th:href="'/user/list?pageNum=' + ${items.getTotalPages() - 1}"
            >尾页</a
          >
        </li>
        <li>
