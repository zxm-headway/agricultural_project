  ...
  >
          ...
          <tr th:each="item : ${items}">
            ...
            <td th:if="${session.user.role!=2}">
              <a
                ...
                th:href="@{/crops/update(id=${item.id})}"
                >修改</a
              >
              <a
                class="btn btn-sm btn-danger"
                th:href="@{/crops/delete(id=${item.id})}"
                >删除</a
              >
            </td>
          </tr>
    ...
    <div class="modal-footer">
      <ul class="pagination pagination-sm">
        <!-- 首页 -->
        <li>
          <a th:href="'/crops/list?pageNum=0'">首页</a>
        </li>
        <!-- 上一页 -->
        <li th:if="${items.hasPrevious()}">
          <a
            th:href="'/crops/list?pageNum=' + ${items.previousPageable().getPageNumber()}"
            th:text="上一页"
          ></a>
        </li>
        <!-- 中间页 -->
        ...
        >
          <a
            th:href="'/crops/list?pageNum=' + ${pageNum}"
            th:text="${pageNum + 1}"
        ...
        <li th:if="${items.hasNext()}">
          <a
            th:href="'/crops/list?pageNum=' + ${items.nextPageable().getPageNumber()}"
            th:text="下一页"
          ></a>
        </li>
        <!-- 尾页 -->
        <li>
          <a th:href="'/crops/list?pageNum=' + ${items.getTotalPages() - 1}"
            >尾页</a
          >
        </li>
        <li>
